# Aubrey Gatewood 3/1/2024
import sys
from BitVector import *


class sha512():
    def __init__(self, inputFile, hashedFile) -> None:
        self.bv = BitVector(textstring = open(inputFile).read())
        self.outFile = open(hashedFile, "w")
        

    def hash(self, infile, outfile):
        '''
        message size can be up to 2^128 bits
        block size 1024 bits
        word size 64 bits
        message digest size 512 bits (size of hashcode)
        security bits: 256
        '''
        ...
        #     tempstring = INFILE.read()
        #     print(tempstring)
        print(infile)
        inputString = BitVector(filename = infile)
        print('input string is: ', inputString)
        inSize = inputString.size
        print("insize mod 1024: ", inSize % 1024)
    ...
    def sha512(self):
        ...
        length1 = bv1.length()
        howmanyzeros = (896 - length1) % 1024 #need to mod 1024
        zerolist = [0] * howmanyzeros
        bv2 = bv1 + BitVector(bitlist = zerolist)
        bv3 = BitVector(intVal = length, size = 128) #change 64 to 128?
        bv4 = bv2 + bv3
        
        #  Initialize the array of "words" for storing the message schedule for a block of the input message:
        words = [None] * 80 # change 64 to 80
        
        for n in range(0,bv4.length(),1024): # change 512 to 1024 
            block = bv4[n:n+1024] # and here
            ...
            words[0:16] = [block[i:i+64] for i in range(0,1024,64)] # this is the IV
        ...
        self.outFile.write(hash_hex_string)
        self.outFile.close()

if __name__ == "__main__":
    hasher = sha512(sys.argv[1], sys.argv[2])
    # hasher.hash(infile=sys.argv[1], outfile=sys.argv[2])
    hasher.sha512()